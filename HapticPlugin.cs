...
public class HapticPlugin : MonoBehaviour
{
    ...
    bool executeNearestSpring = false;
    public int sphereCount = 1;
    public int sphereCountRed = 1;
    #region DLL_Imports
    ...
    [StackableField]
    public string DeviceIdentifier = "HapticActor_DefaultDevice";
    ...
    private void FixedUpdate()
    {
        if (DeviceHHD >= 0)
        {
            ...
            // this will turn on or off the haptic for the nearest dot object or "waypoint"
            if (Input.GetKeyDown(KeyCode.A)){
                executeNearestSpring = true;
            }
            if (Input.GetKeyDown(KeyCode.S))
            {
                executeNearestSpring = false;
                SpringGMag = 0;
            ...
            if (Input.GetKeyUp(KeyCode.D))
            {
                if (GameObject.FindGameObjectsWithTag("Waypoint").Length > 0){
                    ...
                    {
                        GameObject.FindGameObjectsWithTag("Waypoint")[i].GetComponent<MeshRenderer>().enabled = false;
                    }
                }
            }
            else if (Input.GetKeyUp(KeyCode.F))
            {
                if (GameObject.FindGameObjectsWithTag("Waypoint").Length > 0)
                {
                    for (int i = 0; i < GameObject.FindGameObjectsWithTag("Waypoint").Length; i++)
                    {
                        GameObject.FindGameObjectsWithTag("Waypoint")[i].GetComponent<MeshRenderer>().enabled = true;
                    }
                }
            }
            ...
            // if (GameObject.Find("SpringAnchorTest")){
            //     Debug.Log("bootyhole");
            //     EnableSpringNew();
            // }
    ...
    public void EnableSpring()
    {
        if (executeNearestSpring == true){
            if (GameObject.FindGameObjectsWithTag("Waypoint").Length >0){
                ...
                Vector3 hapticColliderPosition = hapticColliderTransform.position;
                int nearestIndex = GetNearestWaypointIndex(hapticColliderPosition);
                Debug.Log(gameObjects[nearestIndex]);
                SpringAnchorObj = gameObjects[nearestIndex];
                float SpringGMag = .005f;
                if (SpringAnchorObj != null)
                {
        ...
        else if (executeNearestSpring == false){
            if (GameObject.FindGameObjectsWithTag("Waypoint").Length >0){
                // Get GameObjects with Waypoint tag
                GameObject[] gameObjects = GameObject.FindGameObjectsWithTag("Waypoint");
    ...
    // public void EnableSpringNew()
    // {
    //     GameObject springThing = GameObject.Find("SpringAnchorTest");
    //     if (springThing != null)
    //     {
    //         double magTe = 2;
    //         float facScal = 1.0f;
    //         setSpringValues(DeviceIdentifier, Vector3ToDoubleArray(gameObject.transform.InverseTransformPoint(springThing.transform.position) / facScal), magTe);
    //     }
    //     else
    //     {
    //         setSpringValues(DeviceIdentifier, Vector3ToDoubleArray(SpringGDir), SpringGMag);
    ...
    public void UpdateForceOnCollision(Collision collision)
    {
        ...
        // HapticMaterial hapMat = collision.collider.GetComponent<
        if (hapMat != null)
        {
